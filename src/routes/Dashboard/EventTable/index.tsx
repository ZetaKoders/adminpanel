...
export const useAlunoData = routeLoader$(async () => {
  try {
    ...
    const result = await db.query<evento[]>('SELECT * FROM event');
...
export default component$(() => {
  const eventoData = useAlunoData().value.flat();
  console.log('data from alunosdata', eventoData)
  ...
  aria-describedby={`checkbox-${event.id}`}
  type="checkbox"
  // ... other properties
/>
<label for={`checkbox-${event.id}`} class="sr-only">
  checkbox
</label>
											...
											<td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
												{event.timeStart.toString()}
											</td>
											<td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
												{event.timeEnd.toString()}
											</td>
